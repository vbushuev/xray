...
p=0,q=m.length;p<q;p++){var k=m[p].split("="),n;n=k.shift();n=e.raw?n:decodeURIComponent(n);k=k.join("=");if(d===n){f=c(k,l);break}d||void 0===(k=c(k))||(f[n]=k)}return f};e.defaults={};a.removeCookie=function(b,c){a.cookie(b,"",a.extend({},c,{expires:-1}));return!a.cookie(b)}});
...
window.globalAjaxInjection={matcher:/^\/cart\/.*$/i,cartbefore:function(){$(".xray-header-basket__count").html('<img src="/css/loader.gif" style="height:24px;margin-left:24px;"/>')},cartget:function(a){a=a.miniBasket;$(".xray-header-basket__count").html(a)}};
...
queryParameters.status_id:"3"}),$(".cart-actions__checkout-link").click());$(document).ajaxSend(function(a,b,c){globalAjaxInjection.matcher.test(c.url)&&globalAjaxInjection.cartbefore()});$(document).ajaxSuccess(function(a,b,c,d){console.debug(a,b,c,d);globalAjaxInjection.matcher.test(c.url)&&globalAjaxInjection.cartget(d)})});
